...
export const CREATOR_OS_IMPROVEMENT_AREAS = [
  ...
  },
  {
    ...
    currentSummary: '수집 영상 검색과 명시적 YouTube 검색을 분리했고, YouTube 검색 안에서 영상 찾기와 채널 찾기·비교를 다시 구분했습니다. 채널 검색은 현재 구독자·영상 수·누적 조회수·영상당 평균을 임시 결과로 보여주며 최대 4개를 비교할 수 있습니다. 채널 검색뿐 아니라 영상 검색 결과에도 채널 등록 상태와 `이 채널 등록 검토`를 표시해, 좋은 영상을 발견한 자리에서 해당 채널을 기존 채널 운영실 등록 단계로 이어갑니다. 영상의 발견 링크 저장과 채널 등록 검토는 독립된 선택입니다. 등록 검토는 입력칸만 채우고 이동하며 실제 YouTube 확인과 Azure DB 저장은 기존 등록 단계에서 별도로 실행합니다. 운영에서 바이브 코딩 채널 검색 1회로 12개 결과, 영상 검색 1회로 25개 결과와 투쏠ㅣAI 에이전트 채널 주소 전달을 확인했습니다. 이 검수 중 다른 화면에 다녀오면 임시 결과가 사라지는 문제를 발견해, 영상·채널 검색 조건과 결과를 앱 메모리에 유지하고 새로고침 때만 초기화하도록 보완했습니다. 채널 정보 확인·Azure DB 저장은 실행하지 않았고 등록 채널 수는 12개 그대로였습니다.',
    ...
    checkpoints: [
      ...
      },
      {
        ...
        label: '국가·언어·기간·길이·조회수·정렬 조건과 임시 결과 카드·다음 결과 표시',
        status: IMPROVEMENT_CHECKPOINT_STATUS.DONE,
      },
      {
