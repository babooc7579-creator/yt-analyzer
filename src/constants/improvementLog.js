...
export const CREATOR_OS_IMPROVEMENT_AREAS = [
  ...
  },
  {
    ...
    priority: 'P2',
    lastReviewedAt: '2026-08-10',
    currentSummary: '수집 영상 목록은 전체 검색·검토, 채널 태그별 보기는 같은 수집 영상 정보를 채널 태그 기준으로 좁히는 전용 보기, 또터또 탐색은 오래된 고성과 후보 판단을 담당합니다. 채널 태그별 보기에서 선택 채널을 바꾸고 곧바로 전체 목록으로 자동 이동하면 이전 범위의 영상이 잠시 새 태그 결과처럼 보일 수 있어 자동 이동은 추가하지 않습니다. 대신 `이 태그 채널로 조회 준비 → 수집 영상 목록 불러오기(Azure DB) → 전체 수집 영상에서 검토` 순서를 화면에 표시합니다. 수집 영상 목록의 모바일 도구는 기본 필터·정렬·보기와 선택 상태의 세 묶음으로 재배치하고 검색 입력이 남은 너비를 사용하도록 정리했습니다. 현재 선택 채널 범위의 Azure DB 조회가 성공한 경우에만 영상·필터 결과·선택 수·전체 수를 표시하며, 채널 선택 변경·전체 해제·조회 실패 때는 이전 범위 영상을 숨깁니다.',
    targetSummary: '메뉴 이름과 화면 안내만으로 전체 목록·전용 필터·독립 판단 화면의 차이를 이해할 수 있게 유지합니다.',
    nextAction: '배포 후 운영 화면에서 채널 A 조회 → 채널 B 선택의 순서로 이전 영상이 숨겨지고 Azure DB 재조회 안내만 남는지 확인합니다. 같은 채널 범위의 조회 완료 결과가 화면 이동 후 유지되는지도 함께 확인합니다.',
    decisions: [
