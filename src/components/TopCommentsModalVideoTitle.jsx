import { getTopCommentsModalVideoTitleViewProps } from '../utils/topComments';

export default function TopCommentsModalVideoTitle({ videoTitle }) {
  const viewProps = getTopCommentsModalVideoTitleViewProps(videoTitle);

  return (
    <div className="p-4 bg-indigo-50/50 border-b border-indigo-100 text-sm font-medium text-indigo-900 line-clamp-1">
      {viewProps.prefix} {viewProps.videoTitle}
    </div>
  );
}
