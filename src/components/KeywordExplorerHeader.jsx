    ...
    <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
      <div className="min-w-0">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
          이미 온라인 저장소(Azure DB)에 보관된 수집 영상 정보의 제목과 채널명만 검색합니다. 검색·필터·정렬만으로 YouTube API를 새로 호출하지 않습니다.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          title="수집 영상 목록 화면으로 이동합니다. 이동만으로 API 호출이나 데이터 변경은 없습니다."
        >
          <BookOpen className="h-4 w-4" /> 수집 영상 목록 화면
        </button>
        <button
          type="button"
          ...
          title={selectedChannelCount > 0
            ? `선택한 채널 ${selectedChannelCount}개의 수집 영상을 온라인 저장소(Azure DB)에서 조회합니다. YouTube API는 호출하지 않습니다.`
            : '먼저 오늘 볼 채널 또는 채널 목록에서 채널을 선택하세요.'}
        >
          {loading
            ? <Loader2 className="h-4 w-4 animate-spin" />
            : <Database className="h-4 w-4" />}
          {loading ? '수집 영상 불러오는 중...' : '수집 영상 목록 불러오기'}
        </button>
      </div>
    </header>
  );
