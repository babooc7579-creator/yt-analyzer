            ...
            return (
              <tr key={video.videoId} className={`group transition-all ${isChecked ? 'bg-indigo-50 ring-1 ring-indigo-200' : isStrongReaction || isTtoTto ? 'bg-rose-50/70 ring-1 ring-rose-100 hover:ring-rose-200' : 'bg-white hover:bg-slate-50 ring-1 ring-slate-100 hover:ring-slate-200'}`}>
                <td className="px-4 py-5 text-center rounded-l-2xl">
                  <button onClick={() => toggleCheckVideo(video.videoId)} title="AI 리메이크 프롬프트에 포함할 제작 검토 후보로 선택" className="focus:outline-none rounded-lg p-1 hover:bg-white transition-colors">
                ...
                <td className="px-2 py-5 text-center">
                  <button onClick={() => toggleScrapVideo(video)} title="스크랩 소재로 저장/해제" className="p-2 rounded-full hover:bg-yellow-100 transition-colors">
                ...
                <td className="px-4 py-5 min-w-[520px]">
                  <div className="flex gap-5">
                    <img src={video.thumbnail} alt="" className="w-36 h-20 object-cover rounded-xl shadow-sm border border-slate-200 shrink-0 bg-slate-100" />
                    <div className="flex flex-col justify-center min-w-0">
                      ...
                      <a href={`https://youtube.com/watch?v=${video.videoId}`} target="_blank" rel="noreferrer" className="text-base font-extrabold text-slate-900 hover:text-indigo-600 line-clamp-2 leading-snug mb-2">{video.title}</a>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className="text-[11px] bg-slate-100 text-slate-600 px-2 py-1 rounded-full border border-slate-200 font-semibold">{LANGUAGES.find((language) => language.code === video.language)?.label || '🌐'}</span>
                        {video.isShorts ? (
                        ...
                        <button onClick={() => fetchTopComments(video.videoId, video.title)} title="YouTube API로 댓글 Top 10을 조회합니다." className="text-[11px] bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-2 py-1 rounded-full font-bold border border-indigo-100 flex items-center gap-1 transition-colors">
                ...
                <td className="px-4 py-5 text-right">
                  <div className={`inline-flex min-w-[110px] flex-col rounded-xl border px-3 py-2 shadow-sm ${isStrongReaction ? 'bg-rose-600 border-rose-600 text-white' : video.multiplier >= TTOTTO_MIN_MULTIPLIER ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'bg-white/80 border-slate-200 text-slate-600'}`}>
                    <span className={`text-[10px] font-bold ${isStrongReaction ? 'text-rose-100' : 'text-slate-400'}`}>대박지수</span>
                ...
                <td className="px-4 py-5 text-right">
                  <div className="inline-flex min-w-[110px] flex-col rounded-xl bg-white/80 border border-slate-200 px-3 py-2 shadow-sm">
                    ...
                    <span className="text-[10px] text-slate-400">👍 {video.like_count.toLocaleString()}</span>
                  </div>
                </td>
                <td className="px-4 py-5 text-right rounded-r-2xl">
                  <div className={`inline-flex min-w-[120px] flex-col rounded-xl border px-3 py-2 shadow-sm ${video.daysOld >= TTOTTO_MIN_DAYS_OLD ? 'bg-orange-50 border-orange-100 text-orange-700' : 'bg-white/80 border-slate-200 text-slate-600'}`}>
                    ...
                    <span className="text-base font-extrabold">{video.daysOld}일 전</span>
