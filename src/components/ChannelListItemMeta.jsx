...
export default function ChannelListItemMeta({
  ...
  return (
    <div className="flex items-center gap-x-2 gap-y-1 flex-wrap mt-1">
      ...
      <span className={gradeBadgeProps.className} title={gradeBadgeProps.title}>{gradeBadgeProps.label}</span>
      <span className={statusBadgeProps.className} title={statusBadgeProps.title}>{statusBadgeProps.label}</span>
      {stats.map((stat) => (
