...
export default function VideoCard({
  ...
  const thumbnailHeightClass = showWorkPanel ? 'min-h-[430px]' : 'min-h-[520px]';

  return (
    <div className={`group overflow-hidden rounded-2xl border shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md ${isChecked ? 'border-indigo-300 bg-indigo-50' : isCandidate ? 'border-rose-100 bg-white' : 'border-slate-200 bg-white'}`}>
      <div className={`relative overflow-hidden bg-slate-100 ${thumbnailHeightClass}`}>
        ...
        <div className="absolute right-3 top-3 flex gap-2">
          ...
          <button onClick={() => onToggleScrap(video)} title="스크랩 소재로 저장/해제" className="rounded-full bg-white/90 p-2 shadow-sm transition-colors hover:bg-yellow-50">
            <Star className={`w-5 h-5 ${isSaved ? 'fill-yellow-400 text-yellow-400' : 'text-slate-400 group-hover:text-yellow-400'}`} />
          </button>
        </div>
      </div>
      <div className={`${showWorkPanel ? 'p-5' : 'p-4'}`}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <div className={`${showWorkPanel ? 'p-3' : 'p-2.5'} rounded-xl border ${isStrongReaction ? 'border-rose-500 bg-rose-600 text-white' : video.multiplier >= TTOTTO_MIN_MULTIPLIER ? 'border-indigo-100 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
          ...
          <div className={`${showWorkPanel ? 'p-3' : 'p-2.5'} rounded-xl border ${video.daysOld >= TTOTTO_MIN_DAYS_OLD ? 'border-orange-100 bg-orange-50 text-orange-700' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
