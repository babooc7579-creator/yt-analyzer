    ...
    <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          title="채널 목록 화면으로 이동합니다. 이동만으로 온라인 저장소(Azure DB) 조회나 YouTube API 호출은 실행되지 않습니다."
          aria-label="채널 목록 화면으로 이동, 온라인 저장소(Azure DB) 조회 및 YouTube API 호출 없음"
        >
        ...
        <button
          ...
          title="선택된 채널의 영상을 온라인 저장소(Azure DB)에서 조회합니다. YouTube API를 호출하지 않습니다."
          aria-label={loading
        ...
        >
          ...
          {loading ? '수집 영상 불러오는 중...' : `수집 영상 목록 불러오기 (${selectedChannelCount})`}
        </button>
      </div>
    </header>
  );
