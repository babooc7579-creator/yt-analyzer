...
export const PRODUCTION_VIDEO_STATUS_HELP_TEXT =
  '오늘 집중과 제작 상태 변경은 온라인 저장소(Azure DB)의 판단 기록에 저장합니다. YouTube API를 새로 호출하지 않습니다.';

const MOVE_ACTION_COPY = {
  [PRODUCTION_STATUS.CANDIDATE]: {
    ariaAction: '제작 후보 상태로 변경, 온라인 저장소(Azure DB)의 판단 기록 저장, YouTube API 호출 없음',
    label: '제작 후보로',
    title: '제작 진행 상태를 제작 후보로 변경해 온라인 저장소(Azure DB)의 판단 기록에 저장합니다. YouTube API를 새로 호출하지 않습니다.',
  },
  [PRODUCTION_STATUS.ACTIVE]: {
    ariaAction: '제작 중 상태로 변경, 온라인 저장소(Azure DB)의 판단 기록 저장, YouTube API 호출 없음',
    label: '제작 중으로',
    title: '제작 진행 상태를 제작 중으로 변경해 온라인 저장소(Azure DB)의 판단 기록에 저장합니다. YouTube API를 새로 호출하지 않습니다.',
  },
  [PRODUCTION_STATUS.DONE]: {
    ariaAction: '업로드 완료 상태로 변경, 온라인 저장소(Azure DB)의 판단 기록 저장, YouTube API 호출 없음',
    label: '업로드 완료',
    title: '제작 진행 상태를 업로드 완료로 변경하고 완료일을 온라인 저장소(Azure DB)의 판단 기록에 저장합니다. YouTube API를 새로 호출하지 않습니다.',
  },
};
...
export const getProductionVideoFocusActionCopy = ({ isFocused = false, videoTitle } = {}) => {
  ...
  return isFocused
    ? {
      ariaLabel: `${displayTitle} 오늘 집중 고정 해제, 온라인 저장소(Azure DB)의 판단 기록 저장, YouTube API 호출 없음`,
      label: '집중 해제',
      title: '오늘 집중 고정만 해제하고 제작 후보 상태는 유지합니다. 온라인 저장소(Azure DB)의 판단 기록에 저장하며 YouTube API를 새로 호출하지 않습니다.',
    }
    : {
      ariaLabel: `${displayTitle} 오늘 집중으로 고정, 온라인 저장소(Azure DB)의 판단 기록 저장, YouTube API 호출 없음`,
      label: '오늘 집중',
...
export const getProductionVideoDraftSaveButtonProps = ({
  ...
  let label = '저장됨';
  let title = '제작 제목, 메모, 업로드 예정일이 온라인 저장소(Azure DB)의 판단 기록에 저장된 상태입니다. YouTube API를 새로 호출하지 않습니다.';

  if (isSaving) {
    label = '저장 중...';
    title = '제작 제목, 메모, 업로드 예정일을 온라인 저장소(Azure DB)의 판단 기록에 저장하는 중입니다. YouTube API를 새로 호출하지 않습니다.';
  } else if (!hasSaveTarget) {
    label = '저장 대상 없음';
    title = '저장할 영상 ID가 없어 온라인 저장소(Azure DB) 저장을 실행하지 않습니다.';
  } else if (isDirty) {
    label = '변경사항 저장';
    title = '제작 제목, 메모, 업로드 예정일을 온라인 저장소(Azure DB)의 판단 기록에 저장합니다. YouTube API를 새로 호출하지 않습니다.';
  }

  return {
    ariaLabel: `${displayTitle} 제작 메모 온라인 저장소(Azure DB)의 판단 기록에 저장, YouTube API 호출 없음`,
    disabled: !canSave,
    label,
    title,
  };
...
export const getProductionVideoDraftStatusBadgeProps = ({
  ...
  if (isSaving) {
    return {
      label: '저장 중',
      title: '변경 내용을 온라인 저장소(Azure DB)의 판단 기록에 저장하는 중입니다.',
  ...
  if (saveState === 'error') {
    return {
      label: '저장 실패',
      title: '온라인 저장소(Azure DB) 저장에 실패했습니다. ‘변경사항 저장’ 버튼을 다시 눌러 주세요.',
  ...
  if (isDirty) {
    return {
      label: '저장 전',
      title: '입력한 변경이 아직 온라인 저장소(Azure DB)의 판단 기록에 저장되지 않았습니다.',
      tone: 'bg-amber-100 text-amber-700',
    };
  }
  ...
  if (saveState === 'saved') {
    return {
      label: '저장 완료',
      title: '현재 제작안이 온라인 저장소(Azure DB)의 판단 기록에 저장됐습니다.',
...
export const getProductionVideoMoveStatusViewProps = ({
  ...
  errorMessage: moveState === 'error'
    ? '온라인 저장소(Azure DB) 상태 저장 실패. 저장 완료 처리하지 않았습니다. 다시 눌러 주세요.'
    : '',
