...
export const DISCOVERY_LINK_SAVING_MESSAGES = {
  create: '새 발견 링크를 온라인 발견함(Azure DB)에 저장하는 중입니다.',
  update: '링크 변경 사항을 온라인 발견함(Azure DB)에 저장하는 중입니다.',
  update_status: '검토 상태를 온라인 발견함(Azure DB)에 저장하는 중입니다.',
  update_rights: '권리 상태 표시를 온라인 발견함(Azure DB)에 저장하는 중입니다.',
  update_text: '제목과 메모를 온라인 발견함(Azure DB)에 저장하는 중입니다.',
  delete: '온라인 발견함(Azure DB)에서 링크 기록을 삭제하는 중입니다.',
};
...
export const getDiscoveryActionError = (error, fallbackMessage, actionLabel = '저장') => {
  ...
  return `${message} 온라인 저장소(Azure DB) ${actionLabel} 완료 처리하지 않았습니다. 연결을 확인한 뒤 다시 시도해 주세요.`;
};

export const getDiscoveryLinkCreatedNotice = (link) => (
  `${getDiscoveryLinkName(link)} 링크를 온라인 발견함(Azure DB)에 저장했습니다.`
);

export const getDiscoveryLinkDeletedNotice = (link) => (
  `${getDiscoveryLinkName(link)} 링크 기록을 온라인 발견함(Azure DB)에서 삭제했습니다.`
);
