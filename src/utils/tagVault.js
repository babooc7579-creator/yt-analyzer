...
export const getTagVaultEmptyState = ({
  ...
  if (channelCount === 0 || tagCount === 0) {
    return {
      action: 'channels',
      actionAriaLabel: '채널 목록 화면으로 이동, 온라인 저장소(Azure DB) 조회 및 YouTube API 호출 없음',
  ...
  if (loadedVideoCount === 0) {
    return {
      ...
      actionLabel: selectedChannelCount > 0 ? '선택 채널 수집 영상 목록 불러오기' : '이 태그 채널 선택',
  ...
  return {
    ...
    actionTitle: hasActiveFilters
      ? '현재 검색어와 길이 필터를 초기화합니다. 온라인 저장소(Azure DB) 데이터나 저장 상태는 바꾸지 않으며 YouTube API를 호출하지 않습니다.'
      : '현재 태그에 속한 채널을 선택합니다. 선택만으로 온라인 저장소(Azure DB) 조회나 YouTube API 호출은 실행되지 않습니다.',
    description: hasActiveFilters
      ? '현재 검색어와 길이 조건에 맞는 수집 영상이 없습니다.'
      : '이 태그 채널의 영상이 현재 불러온 수집 영상 정보에 없습니다. 채널을 선택하고 다시 불러오세요.',
