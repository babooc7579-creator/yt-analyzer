...
describe('loadStoredVideosActionProps utils', () => {
  it('builds enabled DB lookup copy without YouTube API collection wording', () => {
    ...
    expect(props.buttonLabel).toBe('선택 채널 저장 영상 불러오기 (3개)');
    expect(props.actionLabel).toBe('불러오기');
    expect(props.emptyStateLabel).toBe('저장 영상 불러오기');
    expect(props.title).toContain('DB 조회');
    expect(props.title).toContain('YouTube API를 새로 호출하지 않습니다');
    expect(props.actionAriaLabel).toContain('YouTube API 호출 없음');
    expect(props.helperDescription).toContain('Cloud DB');
  ...
  it('keeps loading state explicit about Cloud DB lookup', () => {
    ...
    expect(props.buttonDisabled).toBe(true);
    expect(props.buttonLabel).toBe('Cloud DB에서 저장 영상 불러오는 중');
    expect(props.buttonAriaLabel).toContain('Cloud DB');
