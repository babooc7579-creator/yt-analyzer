...
describe('legacyWorkPanelIntroProps utils', () => {
  it('builds legacy work panel intro copy that separates scan from lookup', () => {
    ...
    expect(props.workflowDescription).toContain('필요할 때만 새 영상 수집');
    expect(props.workflowSteps).toHaveLength(3);
    expect(props.workflowSteps[1].description).toContain('YouTube API');
    expect(props.workflowSteps[2].description).toContain('Cloud DB');
