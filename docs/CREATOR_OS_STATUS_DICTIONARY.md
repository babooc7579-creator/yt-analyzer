...
- 1차 MVP 화면은 `제작 후보`, `제작 중`, `업로드 완료` 3개 칼럼만 사용합니다.
- `production_reviewing`, `production_decided`, `production_on_hold` 기존 값은 저장값을 바꾸지 않고 `제작 후보` 칼럼에 표시해 누락을 막습니다.
- 위 여섯 제작 상태는 모두 이미 제작 흐름에 들어간 기록이므로 `오늘의 레이더` 후보에서는 숨기고 `제작 후보함`에서 계속 확인합니다.
- `focusPinnedAt`은 상태값이 아닙니다. 제작 후보 중 사용자가 오늘 집중으로 고른 영상과 고정 순서를 Cloud에 보존하는 선택적 시각 필드입니다.
- 오늘 집중 고정과 해제는 기존 제작 상태를 바꾸지 않으며, `production_decided`를 대신 사용하지 않습니다.
- 고정 영상은 사용자가 해제하거나 제작 중/업로드 완료로 이동할 때까지 유지하며 날짜 변경만으로 자동 초기화하지 않습니다.
