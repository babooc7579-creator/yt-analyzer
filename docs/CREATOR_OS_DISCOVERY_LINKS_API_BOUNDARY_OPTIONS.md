...
- 2026-07-03 이후 `status: candidate` 기반 제작 후보함 연결은 부분 구현됐지만, 별도 제작 프로젝트 모델과 완전히 연결되지는 않습니다.
...
### 선택지 B: 별도 제작 프로젝트 연결 포함

발견 링크를 단순 `status: candidate` 상태값이 아니라 별도 제작 프로젝트 모델까지 연결합니다.
...
- 발견함은 수동 링크 저장, 메모, 상태 변경, URL 복사, `status: candidate` 기반 제작 후보함 연결까지 1차로 구현합니다.
- 별도 제작 프로젝트 모델, 로컬 파일 연결, 자동 메타데이터 수집은 후속 작업으로 둡니다.
...
3. 발견함 첫 화면은 "최소 발견함"으로 시작하고, `status: candidate` 기반 제작 후보함 연결까지 확장됐습니다. 로컬 파일, 자동 메타데이터, 별도 제작 프로젝트 모델은 아직 확장하지 않습니다.
