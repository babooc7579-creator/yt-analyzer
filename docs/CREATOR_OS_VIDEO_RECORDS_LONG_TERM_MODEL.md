...
1. `status: new` 처리 정책은 2026-07-12 감사에서 정리했습니다.
   - 현재 프론트와 백엔드의 실제 상태 상수 및 저장 흐름에서는 `new`를 생성하지 않습니다.
   - 기존 Cloud record에 `new`가 남아 있으면 데이터 보호를 위해 값 자체는 보존합니다.
   - 현재 영상 검토 상태나 제작 상태로 해석하지 않으며, 레이더에서 처리 완료 항목으로 숨기지 않습니다.
   - 자동으로 `unseen`이나 다른 상태로 마이그레이션하지 않습니다.
   - 이 호환 정책은 `src/constants/status.test.js`로 고정합니다.
2. `ProductionKanban` 내부의 3단계 칼럼과 장기 제작 상태 후보 연결은 2026-07-12 감사에서 정리했습니다.
   - 현재 화면과 상태 변경 버튼은 `제작 후보`, `제작 중`, `업로드 완료` 3단계만 생성합니다.
   - 장기 후보값인 `production_reviewing`, `production_decided`, `production_on_hold`는 현재 화면에서 새로 생성하지 않습니다.
   - 기존 Cloud record에 이 값이 남아 있으면 데이터가 화면에서 사라지지 않도록 `제작 후보` 칼럼에 표시합니다.
   - 이 매핑은 저장된 원래 상태값을 바꾸거나 자동 마이그레이션하지 않는 표시 정책입니다.
   - 3단계를 확장하는 일은 1차 완성 이후 별도 제품 결정으로 남깁니다.
3. 레이더/보관함/제작 칸반의 상태 표시명 감사는 2026-07-12에 완료했습니다.
   - 레이더 판단값은 공통 사전의 `봤음`, `나중에 보기`, `제작 후보`, `제외`를 사용합니다.
   - 영상 카드와 목록의 후보 표시는 `제작 후보`로 통일되어 있습니다.
   - 레거시 `later`와 현재 `watch_later`는 데이터 호환을 위해 모두 `나중에 보기`로 표시합니다.
   - 소재를 이미 활용했다는 `used`는 `사용함`, 제작을 마치고 올렸다는 `uploaded`는 `업로드 완료`로 구분합니다.
   - 공통 표시명 정책은 `src/constants/status.test.js`로 고정합니다.
