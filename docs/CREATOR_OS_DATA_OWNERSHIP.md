...
- 프론트는 `statusIds`를 일부 사용하므로, 백엔드 저장 구조와 불일치 가능성이 있습니다.
...
| `videoUserRecords` | Cloud DB + localStorage 보조 | Cloud DB | Cosmos `videos` container 안의 `docType: video_user_record` | 장기적으로 Cloud DB 기준. 현재 단계는 `status` 유지 + `statusIds` 보존 | 현재 캐시/복구/호환 역할 | 영상별 사용자 판단 상태, 노트, 제작 관련 필드 저장 | 부분 구현 | 중간. `status`와 `statusIds`의 장기 분리 모델은 아직 미정 |
...
- 영상 수가 커지면 성능과 비용 문제가 생길 수 있습니다.
...
- 백엔드 저장 중심: 단일 `status`
- 프론트: `statusIds`를 함께 해석하고 저장하려는 흐름 있음
- localStorage key: 기존 호환/캐시 역할

목표 방향:

- 장기 기준은 Cloud DB입니다.
- 단일 `status`와 복수 `statusIds`의 관계를 상태값 사전에서 먼저 확정해야 합니다.
- 지금 당장 백엔드 schema를 바꾸지 않습니다.

충돌 위험:

- 높음.
- 프론트에서 여러 상태를 저장했다고 생각해도 백엔드가 일부 필드를 버릴 수 있습니다.
...
- localStorage는 기존 스크랩 복구/호환 역할

목표 방향:

- 장기 기준은 Cloud DB입니다.
- localStorage는 임시 복구/호환 용도로만 봅니다.
...
2. localStorage는 임시 캐시, 복구, 기존 사용자 호환 용도로만 봅니다.
3. Cloud DB와 localStorage가 충돌하면 장기적으로 Cloud DB를 기준으로 삼습니다.
...
- `videoUserRecords` 초기 로딩/복구
- 기존 스크랩북 데이터 클라우드 이전 보조
