...
- 2026-07-20 최신 기준 `npm test -- --reporter=dot`는 테스트 파일 215개, 테스트 932개가 통과합니다. 캘린더에서 제작 후보 한 건을 찾는 화면 의도, 홈 레이더 빈 상태의 다음 행동, 영상 카드의 게시일·경과일·후보 이유·주요 행동 순서와 고화질 썸네일 대체 순서, 설정 복구와 레이더 복귀 흐름이 hook, utility, component 렌더링 테스트에 포함됩니다. 또한 스크랩북/레이더의 영상 후보가 제작 후보, 오늘 집중, 업로드 일정으로 이어지고 발견 링크 후보는 별도 저장 모델을 유지하는 통합 흐름 계약을 한 파일에서 검증합니다.
...
- 2026-07-08 후속 안정화에서 홈 화면 빠른 작업 버튼 문구, 홈 화면 내비게이션 props, 홈 레이더 작업 흐름 안내, 저장 영상 안내 카드, 태그별 새 영상 수집 안내, 채널 추가 안내 문구, 제작 후보함의 발견 링크 설명, 제작 칸반 요약/범례 문구, 레이더 후보 상태 문구, 발견함 헤더/필터 빈 상태/재조회 문구, 스크랩북 헤더 문구, 제작 상태/저장 문구, 선택 영상 AI 요청문 복사 바, 제작 후보함 발견 링크 액션, 레이더 후보 헤더/처리 기록 문구, 채널 추가 헤더/일괄 결과/카테고리/빈 상태 문구, 채널 URL 목록 복사/로딩 문구, 채널 미리보기·언어 선택·카테고리 도움말 문구, 채널 메모 모달·메타 컨트롤·일괄 실패 문구, 영상 툴바 검색/필터/정렬/보기/터또터 버튼 문구, 영상 카드/리스트 행 배지·복사·통계·AI 요청문 선택 문구, 공통 복사 버튼/좌측 메뉴/작업 헤더/작업 탭/준비중 화면 문구, 발견함 수동 링크 폼/필터/상태 변경/중복 링크 안내 문구, 제작 후보함 지표/일정/영상 카드/입력 필드/외부 링크 버튼 문구, 레이더 후보 카드/지표/스크랩/요약 문구, 홈 레이더 히어로/운영 기준/후보 워크플로우/다음 행동 문구, 스크랩북 영상 카드/썸네일/통계 문구, 댓글 Top 10 모달 제목/로딩/빈 상태 문구, 기존 작업 화면 안내/선택 영상 바 문구, 홈 다음 행동의 저장 영상/채널 목록 이동, 발견함의 제작 후보함 이동 문구를 테스트로 보강했습니다.
...
61. `src/utils/legacyDashboardTabViewProps.js`
   - 기존 대시보드 탭 props 구성
   - 수집/필터/정렬/영상 액션 props 연결
   - 1차 테스트 추가 완료
...
64. `src/utils` 1차 순수 유틸 테스트 목록 점검
   - 누락된 순수 유틸 파일 확인
   - 다음 테스트 단계 분리 여부 검토
   - 2026-07-08 점검 완료: `src/utils` 순수 유틸 91개와 테스트 파일 91개가 일치
   - 전체 테스트 파일은 `src/utils` 91개, `src/services` 9개, `src/hooks` 1개, `src/constants` 2개로 총 103개입니다.
...
65. 안정화 후속 테스트 보강
   ...
   - 제작 후보함의 발견 링크 섹션 설명
   - 제작 칸반 요약/범례 문구
   - 레이더 후보 완료/빈 상태/제작 후보 버튼 문구
   - 발견함 헤더/필터 빈 상태/재조회 버튼/수동 링크 폼/상태 변경/중복 링크 안내 문구
   - 스크랩북 헤더 문구
   - 제작 칸반 상태 이동/저장 상태 문구
   - 선택 영상 AI 요청문 복사 바 문구
   - 제작 후보함 발견 링크 복사/열기/수정/이동 상태 문구
   - 레이더 후보 헤더/처리 기록 문구
   - 채널 추가 헤더/일괄 결과/카테고리/채널 목록 빈 상태 문구
   - 채널 URL 목록 복사/채널 목록 로딩 문구
   - 채널 미리보기 완료/첫 기록 메모/언어 선택/태그 선택/카테고리 도움말 문구
   - 채널 메모 모달/채널 메타 컨트롤/일괄 저장 실패 문구
   - 영상 툴바 검색/필터/정렬/보기/터또터 버튼 문구
   - 영상 카드/리스트 행 배지/복사/통계/AI 요청문 선택 문구
   - 공통 복사 버튼/좌측 메뉴/작업 헤더/작업 탭/준비중 화면 문구
   - 제작 후보함 지표/일정/영상 카드/입력 필드/외부 링크 버튼 문구
   - 2026-07-08 1차 완료: 전체 테스트 파일 108개, 테스트 481개가 통과합니다.
