...
## 11. 2026-08-08 자동 수집·DB 쓰기 경계 보정

- 기존 야간 Timer는 매일 한국시간 새벽 3시에 실행되지만, 이제 `status: active`만으로는 자동 수집하지 않습니다.
- 새 채널의 기본 `collectionMode`는 `manual`입니다. 야간 Timer는 `status: active`이면서 `collectionMode: auto`로 명시된 채널만 처리합니다.
- 사용자가 실행하는 전체·태그·선택 채널 수동 수집은 기존처럼 활성 채널을 대상으로 하며 자동 수집 설정과 분리합니다.
- 통계 갱신 영상은 통계와 대박지수를 합쳐 한 번만 저장합니다. 통계 갱신 대상이 아니고 대박지수도 같은 기존 영상은 다시 저장하지 않습니다.
- 자동 실행 로그는 후보 영상 전체 배열을 남기지 않고 대상 채널 수, 실패 수, 신규 영상 수, 통계 갱신 수, 실제 영상 저장 수만 남깁니다.
- 이 변경은 새 Azure 자원, Cosmos DB container, YouTube API 호출 종류나 자동 실행 횟수를 추가하지 않습니다.
- 백엔드 PR #28 배포 후 2026-08-09 첫 야간 실행이 대상 채널 0·실패 0·신규 0·통계 갱신 0·영상 저장 0으로 341ms 만에 완료됐습니다. 같은 15분 구간의 Cosmos 429와 Upsert는 모두 0건이었습니다.
