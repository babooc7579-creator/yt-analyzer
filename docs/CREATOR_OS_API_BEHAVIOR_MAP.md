...
| 키워드 영상 검색 | `searchYoutubeVideos` | `GET /youtube-search?q=...` | YouTube API 조회 | 예 | 아니오 | 아니오 | 아니오 | 가능 | 검색 버튼과 다음 결과 버튼에서만 실행. `쇼츠 후보`는 API의 4분 미만 결과에서 앱이 3분 이하만 남김. 제목·설명에 Shorts 표기가 있으면 `가능성 높음`, 없으면 `확인 필요`로 화면에서만 구분하되 화면비와 공식 분류는 원본 확인 필요. 임시 결과 중 선택한 영상만 별도 발견 링크 저장 가능. 기존 `GET /discovery-links` 결과의 영상 ID·YouTube URL을 대조하며 조회 중·실패 시 중복 확인 전 저장 차단 |
...
### 4.4.1 sessionStorage 검색 복구

- YouTube 영상·채널 검색의 조건, 임시 결과, 화면 필터와 선택만 같은 탭에서 최대 6시간 복구합니다.
- 새로고침 복구는 YouTube API 재검색, Azure DB 조회·저장, 새 영상 수집을 실행하지 않습니다.
- 브라우저 탭을 닫거나 6시간이 지나면 복구하지 않으며, Azure DB 기준 데이터와 병합하지 않습니다.
