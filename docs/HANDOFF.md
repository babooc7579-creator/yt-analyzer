...
2026-07-13 기준으로 초기의 `App.jsx` 집중 구조는 상당 부분 해소되었고, 주요 데이터/상태 흐름에는 회귀 테스트 안전망이 추가되었습니다.
...
- `업로드 캘린더`는 기존 Cloud 영상별 사용자 기록의 `targetPublishDate`를 읽어 제작 후보 일정을 월별로 표시합니다. 일정 수정은 기존 제작 후보 편집 흐름으로 연결합니다.
- 제작 후보함은 영상 제목·채널·제목 초안·메모 검색과 진행 단계 필터를 지원하고, 현재 카드의 제목·메모·일정·원본 URL을 `작업 묶음`으로 로컬 클립보드에 복사합니다.
...
- 최근 전체 검증 기준은 `npm.cmd test`, `npm.cmd run build`, `git diff --check` 통과입니다. 2026-07-14 기준 테스트는 `190개 파일 / 814개 테스트`까지 통과했습니다.
