...
2026-07-12 기준으로 초기의 `App.jsx` 집중 구조는 상당 부분 해소되었고, 주요 데이터/상태 흐름에는 회귀 테스트 안전망이 추가되었습니다.
...
- 최근 전체 검증 기준은 `npm.cmd test`, `npm.cmd run build`, `git diff --check` 통과입니다. 2026-07-12 기준 테스트는 `171개 파일 / 747개 테스트`까지 통과했습니다.
