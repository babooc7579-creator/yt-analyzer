...
- 최근 전체 검증 기준은 `npm.cmd test -- --reporter=dot`, `npm.cmd run build`, `npm.cmd audit --omit=dev` 통과입니다. 2026-07-11 기준 테스트는 `165개 파일 / 731개 테스트`까지 통과했습니다.
- 이 검증은 "버그가 절대 없다"는 의미가 아니라, 핵심 흐름을 고치다가 기존 동작이 깨지는 위험을 훨씬 빨리 발견할 수 있는 상태라는 뜻입니다.
- 홈 레이더 흐름, 발견함 빈 상태/필터/헤더 액션, 스크랩북/참고 보관함, 제작 후보함 빈 상태/다음 행동과 저장 실패 안내, 제작 후보 카드 준비 체크, 채널 안전 버튼, 영상 카드/리스트 액션, 댓글 Top 10 모달 상태 문구까지 렌더링 테스트로 보호합니다.
