...
- 사이드바는 실제 사용 가능한 12개 메뉴를 기본 표시하고, 준비중 19개는 `향후 기능` 접이식 영역에 보존합니다. 준비중 안내 화면과 제품 로드맵은 삭제하지 않았으며, 접기/펼치기는 API 호출이나 데이터 변경을 실행하지 않습니다.
- 최근 전체 검증 기준은 `npm.cmd test`, `npm.cmd run build`, `git diff --check` 통과입니다. 2026-07-13 기준 테스트는 `181개 파일 / 792개 테스트`까지 통과했습니다.
