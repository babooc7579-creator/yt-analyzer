...
- 최근 전체 검증 기준은 `npm.cmd test -- --reporter=dot`, `npm.cmd run build`, `npm.cmd audit --omit=dev` 통과입니다. 2026-07-11 기준 테스트는 `160개 파일 / 720개 테스트`까지 통과했습니다.
...
4. 테스트가 약한 새 조합 흐름이 있으면 테스트부터 추가합니다. 현재 주요 화면 흐름의 렌더링 안전 문구는 1차 보강이 완료됐습니다.
