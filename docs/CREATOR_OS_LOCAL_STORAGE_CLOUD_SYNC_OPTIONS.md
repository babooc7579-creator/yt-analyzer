...
| scrapbook | Cosmos `videos` container 안의 `docType: scrapbook` | Cloud 기준. localStorage는 장애 시 fallback |
...
→ 성공하면 Cloud records를 화면 기준으로 사용
→ 성공한 Cloud records만 localStorage 캐시로 기록
→ 실패하면 localStorage yt_crm_video_user_records를 임시 fallback으로 표시
```
...
- Cloud 저장 성공 후에만 localStorage 캐시를 갱신합니다.
- Cloud 저장 실패 시 화면에는 임시 반영될 수 있지만, localStorage에 저장 완료처럼 기록하지 않습니다.
...
앱 시작
→ /scrapbook Cloud 조회
→ 성공하면 Cloud videos를 화면 기준으로 사용
→ 성공한 Cloud videos만 localStorage 캐시로 기록
→ 실패하면 localStorage yt_crm_saved_videos를 임시 fallback으로 표시
```
...
| `scrapbook` | Cloud + localStorage 보조 | B 우선, 자동 업로드 금지 | 기존 스크랩 손실 방지는 fallback으로 처리하고, Cloud 이전은 별도 수동 복구 Issue로 분리 |
...
- 스크랩북 local → Cloud 자동 업로드는 하지 않습니다.
- 자동 병합/자동 마이그레이션은 하지 않습니다.

---

## 11. 결정이 필요한 질문

아래 질문은 2026-07-02에 다음처럼 결정했습니다.
...
4. 아니오. 자동 업로드하지 않습니다. 스크랩북 손실 방지는 Cloud 실패 fallback으로만 처리하고, Cloud 이전은 별도 수동 복구/마이그레이션 Issue로 분리합니다.
...
- 자동 마이그레이션 구현
- localStorage 데이터의 Cloud 자동 업로드
- 기존 사용자 데이터 초기화
- Cloud DB schema 변경
