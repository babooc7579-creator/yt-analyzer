...
## 8. 로그인 만료와 작업 흐름 복구 기준

2026-07-20 운영 앱에서 아래 연결 흐름을 확인했습니다.

- 로그인 세션이 만료되면 보호된 Cloud API 요청이 Microsoft 로그인 화면으로 이동할 수 있습니다.
- Microsoft 계정으로 다시 로그인한 뒤 같은 Cloud 조회를 재실행하면 기존 저장 데이터가 유지됩니다.
- 로그인 만료나 Cloud 조회 실패를 `데이터 0개`로 간주하지 않습니다.
- Cloud 조회가 성공해 0개를 반환한 경우에만 정상 빈 결과로 판단합니다.
- Cloud 조회 실패 시 localStorage 기록은 임시 fallback일 뿐이며 Cloud 데이터와 자동 병합하거나 자동 업로드하지 않습니다.
- 제작 후보 영상의 `오늘 집중`을 켠 뒤 화면을 다시 열어 Cloud 보존을 확인했고, 검증 후 집중 상태를 원래대로 해제했습니다.
- 제작 후보가 업로드 캘린더의 `날짜 미정` 항목으로 이어지는 것을 확인했습니다.
- 발견함의 `candidate` 링크가 제작 후보함의 링크 후보로 연결되는 것을 확인했습니다.

로그인 복구는 인증을 다시 얻는 절차이며 새 영상 수집, Cloud 데이터 초기화, localStorage 마이그레이션을 실행하지 않습니다.
