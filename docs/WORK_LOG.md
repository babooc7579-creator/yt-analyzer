...
## 242. 2026-08-10 수집 영상 조회 완료 상태 화면 간 유지

- 운영에서 TechByTosh 1개를 선택하고 Azure DB의 수집 영상 정보 95개를 조회한 뒤 `수집 영상 전체 보기`로 이동했습니다.
- 영상 95개와 또터또 후보 9개는 표시됐지만 선택 채널 개수와 빠른 보기 세 항목은 모두 `조회 전`으로 남는 실제 오류를 확인했습니다.
- 원인은 각 화면의 조회 완료 결과가 화면 내부 상태로만 관리되어 다른 화면으로 이동할 때 사라지는 구조였습니다.
- 조회 결과에 선택 채널 범위 키를 붙여 앱 공통 상태로 유지하고, 같은 채널 범위인 경우에만 오늘의 레이더·수집 영상 목록·채널 태그별 보기·또터또·키워드 탐색에서 재사용하도록 보완했습니다.
- 다른 채널 범위로 바뀌면 이전 조회 결과를 재사용하지 않으므로 오래된 개수를 새 선택으로 오인하지 않습니다.
- `보관함 도구막대`는 실제 역할이 보관함이 아니라 현재 목록 검색·필터·정렬이므로 `영상 검토 도구`로 변경했습니다.
- 실제 앱 범위 269개 테스트 파일·1,232개 테스트와 production build가 통과했습니다.
- 이번 운영 확인은 Azure DB 읽기 1회만 실행했으며 YouTube API 신규 수집과 Azure DB 저장은 실행하지 않았습니다.
- PR #1098 검사·main 병합·Azure Static Web Apps 배포가 성공했습니다.
- 배포 후 TechByTosh를 다시 선택해 Azure DB 영상 정보 95개를 조회하고 수집 영상 목록으로 이동했습니다.
- 선택 채널은 `TechByTosh · 95개`, 빠른 보기는 최근 30일 4개·오래된 인기 3개·또터또 9개로 표시됐으며 `조회 전`은 남지 않았습니다.
- `영상 검토 도구` 명칭과 브라우저 콘솔 오류 없음도 확인했습니다. 운영 재검수는 Azure DB 읽기 1회만 실행했고 YouTube API·DB 저장은 실행하지 않았습니다.

## 243. 2026-08-10 수집 영상 카드 판단 정보 간소화

- 일반 영상 카드마다 반복되던 `비교 참고` 상자를 숨겨 한 화면에서 제목·통계·결정 버튼을 더 빠르게 비교하도록 정리했습니다.
- 또터또 기준이나 강한 참여 반응 등 실제 신호가 있는 영상은 기존 판단 근거를 삭제하지 않고 `검토 이유` 한 줄로 압축했습니다.
- `영상 검토 도구` 상단에 `제작 후보로 → 제작 후보함`, `소재 보관 → 소재 보관함`을 표시해 버튼을 누른 뒤 확인할 화면을 먼저 알 수 있게 했습니다.
- 소재 보관은 `/scrapbook` Azure DB 저장, 제작 후보는 제작 원본 보존 뒤 영상 판단 기록의 제작 후보 상태 저장이라는 기존 흐름을 코드와 회귀 테스트로 다시 대조했습니다.
- 화면 정보 구조만 변경했으며 YouTube API 신규 수집, Azure DB 저장 실행, localStorage 키, endpoint와 데이터 구조는 변경하지 않았습니다.

## 244. 2026-08-10 수집 영상 모바일 필터와 태그 조회 순서 정리

- 수집 영상 도구의 검색·조회수·길이를 기본 필터 묶음으로 배치하고, 정렬과 보기·선택 상태를 각각 구분했습니다.
- 검색 입력은 고정 224px 대신 현재 화면의 남은 너비를 사용해 좁은 화면에서 잘리지 않도록 했습니다.
- 채널 태그별 보기의 `태그 채널 선택`은 실제 다음 행동이 드러나도록 `이 태그 채널로 조회 준비`로 변경했습니다.
- 태그 선택 직후 전체 목록으로 자동 이동하면 이전 조회 영상이 새 선택 범위처럼 보일 수 있어 자동 이동은 추가하지 않았습니다.
- 화면에 `태그 채널로 조회 준비 → 수집 영상 목록 불러오기(Azure DB) → 전체 수집 영상에서 검토` 순서를 표시했습니다.
- 조회 성공 후 실제 0개와 조회 실패·조회 전 상태는 기존 회귀 테스트를 유지합니다.
- 화면 선택과 배치만 변경했으며 Azure DB 자동 조회·저장, YouTube API, localStorage와 데이터 구조는 변경하지 않았습니다.

## 245. 2026-08-10 수집 영상 선택 범위 불일치 방지

- 채널 선택 결과 상태는 선택 범위 키에 맞춰 초기화되지만, 화면에 전달되는 영상 배열은 직전 Azure DB 조회 결과를 유지하는 경로를 확인했습니다.
- 수집 영상 목록에서는 현재 범위 조회 성공 전까지 영상·필터 결과·선택 수·전체 수를 빈 상태로 전달하도록 보완했습니다.
- 채널 변경, 전체 선택 해제, 조회 실패에서는 이전 범위 영상을 숨기고, 같은 범위의 성공 조회와 수집 영상 목록 이외 화면은 그대로 유지합니다.
- 조회 전·실패·성공·전체 선택 해제·다른 화면 보존을 회귀 테스트로 추가했습니다.
- 화면 표시 계산만 변경했으며 Azure DB 조회·저장, YouTube API, localStorage와 원본 데이터는 변경하지 않았습니다.

## 246. 2026-08-10 수집 영상 범위 앱 전체 일치

- 후속 교차 점검에서 오늘의 레이더·채널 태그별 보기·또터또·수집 영상 검색도 직전 공통 영상 배열을 사용할 수 있음을 확인했습니다.
- 현재 선택 채널 키와 Azure DB 조회 성공 키가 일치할 때만 유효한 영상 객체를 앱 중앙에서 각 업무 화면과 파생 수치에 전달합니다.
- 조회 전·실패·전체 선택 해제·다른 범위 결과는 빈 영상 범위로 처리하고, 같은 범위의 성공 결과만 유지합니다.
- 제작 후보·소재 보관·발견 링크 등 별도 Azure DB 사용자 자산은 유지하며, 수집 영상의 화면 범위만 제한합니다.
- 중앙 범위 판정 6개와 컨트롤러 전달 2개 테스트를 추가했고 Azure DB 쓰기·YouTube API·localStorage 변경은 없습니다.

## 247. 2026-08-10 늦은 Azure DB 조회 응답 차단

- 채널 A 조회 중 채널 B로 전환해 B 조회를 완료한 뒤 A 응답이 늦게 도착하는 순서를 점검했습니다.
- 앱 런타임에 조회 요청 번호·선택 범위 키·현재 로딩 소유 요청을 보관하고 가장 최근 요청만 영상·완료 결과·진행 문구를 갱신하도록 했습니다.
- 이전 요청은 성공·실패 어느 경우에도 현재 채널 범위 상태를 덮지 않으며 새 요청이 가진 로딩 상태를 해제하지 않습니다.
- 지연 응답 순서를 자동 테스트로 고정했으며 API endpoint·Azure DB 저장·YouTube API·localStorage·화면 구조는 변경하지 않았습니다.

## 248. 2026-08-11 수집 영상 범위 운영 왕복 검수

- 최신 운영 배포에서 TechByTosh Azure DB 수집 영상 95개를 조회한 뒤 David Fortin으로 선택 범위를 변경했습니다.
- 새 범위를 조회하기 전 공통 수치와 오늘의 레이더·수집 영상 목록·채널 태그별 보기·또터또·수집 영상 기반 키워드 탐색이 모두 0개 조회 전 상태로 바뀌고 이전 영상이 사라졌습니다.
- David Fortin의 Azure DB 수집 영상 251개를 2페이지에서 조회한 뒤 목록 251개·레이더 251개 준비·또터또 후보 20개·키워드 기반 251개가 같은 범위로 표시됐습니다.
- 수집 영상 목록은 첫 60개 지연 표시를 유지했고 운영 브라우저 콘솔 오류는 0건이었습니다.
- Azure DB 읽기 2회 외에 YouTube API·댓글 API·Azure DB 쓰기·삭제·사용자 판단 상태 변경은 실행하지 않았습니다.

## 249. 2026-08-11 모바일 작업 탭과 키워드 후속 동선

- 390×844 수집 영상 목록에서 두 작업 탭의 긴 제목과 `현재 화면` 배지가 여러 줄로 갈라지는 문제를 확인했습니다.
- 모바일에서는 탭을 같은 너비의 두 칸으로 두고 `수집 목록`·`소재 보관`·`제작 후보`로 짧게 표시하며, 데스크톱 전체 명칭과 접근성 이름은 유지합니다.
- 수집 영상 키워드 탐색 헤더에 `또터또 후보 보기`를 추가해 같은 수집 범위의 오래된 강한 반응 영상을 바로 검토할 수 있게 했습니다.
- 현재 키워드 필터는 또터또 화면에 전달되지 않으며, 이 버튼은 화면만 이동하고 Azure DB 조회·저장이나 YouTube API를 실행하지 않습니다.
- Azure DB 불러오기 처리 중 중복 클릭 차단·실패 재시도, 태그 화면의 조회 준비 순서, 제작 후보·소재 보관 결과 위치는 기존 코드와 테스트에서 정상임을 확인해 중복 기능을 추가하지 않았습니다.

## 250. 2026-08-11 태그 조회 행동 중복 제거

- David Fortin 251개 범위의 390×844 운영 화면에서 카드 60개→120개, 리스트 120개, 최근 30일 6개를 확인했습니다.
- 화면 계산인 `copilot` 검색은 211개, 같은 범위의 또터또 후보는 20개였습니다. 이 과정은 YouTube 검색 API를 호출하지 않았습니다.
- `카이온학습` 태그의 9개 채널로 조회 준비하자 직전 251개가 숨겨졌고, Azure DB 읽기 뒤 새 범위 1,743개가 표시됐습니다.
- 조회 준비 상태에서 동일한 Azure DB 불러오기 행동이 상단과 빈 상태에 중복되어 상단 버튼 하나로 통합했습니다.
- 필터가 유지되는 목록 이동 버튼은 `전체 수집 영상` 대신 `수집 영상 목록`으로 바꾸고 현재 필터가 남을 수 있음을 안내했습니다. 필터를 자동 초기화하지는 않습니다.
- 기존 소재 보관 1개와 제작 후보 영상 1개·링크 1개 결과 위치를 읽기 전용으로 확인했습니다.
- 운영 검수는 Azure DB 읽기 2회만 실행했고 YouTube API·댓글 API·Azure DB 쓰기·삭제·사용자 판단 상태 변경은 실행하지 않았습니다.

## 251. 2026-08-11 검색 기준과 대량 태그 표시 명확화

- 수집 영상 키워드 추천어는 제목 토큰만 집계하지만 실제 검색은 제목과 채널명을 함께 확인해 `copilot` 210개와 211개 차이가 생긴 원인을 확인했습니다.
- 추천어의 `영상`을 `제목 영상`으로 바꾸고 검색 결과 수가 달라질 수 있음을 화면에 설명했습니다.
- 수집 영상 목록의 활성 필터에 개수와 함께 검색어·조회수·길이·최근 30일·오래된 인기·또터또 이름을 표시합니다.
- 채널 태그별 보기 결과는 첫 60개만 렌더링하고 60개씩 추가 표시할 수 있게 했습니다. 화면 더 보기는 Azure DB 재조회·저장이나 YouTube API 호출이 아닙니다.
- 소재 보관→제작 후보와 제작 후보→대본 작업실→업로드 캘린더의 역할·중복 방지·동일 기록 전달을 기존 코드와 회귀 테스트로 확인했습니다.
- 조회 성공 0개·실패·조회 전 상태는 기존 안전 분기를 유지하며 데이터·API·localStorage 계약은 변경하지 않았습니다.

## 252. 2026-08-11 임시 흐름 검증 기록의 역할 확정

### 확인

- `Codex 임시 흐름 검증 2026-07-20`은 운영 발견 링크함에서 제작 후보 상태로 남아 있는 Azure DB 기록입니다.
- 2026-07-20 공개 앱 점검 문서에는 발견함의 candidate 링크가 제작 후보함으로 연결되는지 확인한 기록이 있습니다.
- 코덱스 개발 변경 이력은 해당 링크 한 건이 아니라 Git 커밋·Pull Request와 `WORK_LOG`, `HANDOFF`, 개선 기록에 별도로 남습니다.

### 사용자 결정과 반영

- 현재는 개인용 앱이므로 검수 표본이 화면에 보여도 괜찮다는 사용자 결정을 반영했습니다.
- 삭제 대기 체크포인트를 `개인용 운영 표본으로 보존` 완료 상태로 변경했습니다.
- 운영 Azure DB의 링크 내용·상태·태그를 수정하거나 삭제하지 않았고 YouTube API도 호출하지 않았습니다.

## 253. 2026-08-11 YouTube 검색 복합 조건과 페이지 기준 고정

### 확인한 문제

- 영상 검색의 국가·우선 언어·기간·길이·정렬은 YouTube API 요청에 전달되고 최소 조회수는 받은 결과의 화면 필터로만 적용됩니다.
- 최근 7일·30일·60일 검색은 `publishedAfter`를 현재 시각에서 계산합니다.
- 첫 25개 뒤 다음 결과를 요청할 때 이 시각을 다시 계산하면 첫 요청과 다음 페이지 토큰의 검색 기준이 미세하게 달라질 수 있었습니다.

### 보완

- 첫 검색에서 계산한 `publishedAfter`를 적용 조건과 함께 임시 검색 세션에 유지합니다.
- 다음 25개는 첫 검색의 키워드·국가·언어·기간·길이·정렬·시작 시각과 받은 페이지 토큰을 사용합니다.
- 다음 결과 버튼에 첫 검색 조건을 유지하는 API 추가 조회라는 설명을 표시합니다.

### 안전 범위와 검증

- 프론트 검색 유틸·hook·화면·API 서비스 관련 테스트를 통과했습니다.
- 백엔드 전체 모의 테스트에서 국가·언어·기간·길이·페이지 토큰의 정규화와 YouTube 전달 계약을 확인했습니다.
- Azure DB 쓰기·삭제, 발견 링크 저장, YouTube 운영 검색, 백엔드 코드와 endpoint 변경은 실행하지 않았습니다.

## 254. 2026-08-11 검색 영상에서 중요 채널 등록 경계 재점검

### 확인

- 영상 검색 결과의 `이 채널 등록 검토`는 영상 URL이 아니라 `https://www.youtube.com/channel/{채널ID}`를 채널 운영실에 전달합니다.
- 등록 검토 이동은 주소만 미리 채우며 YouTube API·Azure DB 저장·영상 수집을 실행하지 않습니다.
- 채널 운영실의 단일 `YouTube에서 확인`은 실제로 채널 확인 API를 호출하지만, 버튼명만으로 API 사용 단계가 충분히 드러나지 않았습니다.

### 보완

- 단일 확인을 `YouTube API로 확인`, 일괄 등록을 `YouTube API 확인 후 일괄 저장`으로 표시했습니다.
- 단일 확인은 채널 정보 조회만 수행하고 Azure DB 저장·영상 수집은 하지 않으며, 일괄 등록은 API 확인 뒤 Azure DB 저장까지 수행한다는 설명을 구분했습니다.
- 검색 결과 복귀 시 앱 메모리의 조건·결과·선택을 유지하는 기존 경로는 그대로 보존했습니다.

### 안전 범위

- 문구와 회귀 테스트·운영 문서만 변경했습니다.
- 실제 YouTube API 호출, Azure DB 조회·저장, 새 영상 수집은 실행하지 않았습니다.

## 255. 2026-08-12 채널 확인 입력의 모바일 폭 개선

### 확인한 문제

- `YouTube API로 확인` 문구는 390×844에서 잘리지 않았고 화면 가로 넘침도 없었습니다.
- 하지만 입력칸과 버튼을 한 줄에 두면서 버튼이 155px를 차지해 채널 주소 입력칸이 약 86px까지 줄었습니다.
- 긴 채널 주소를 확인하고 수정하기 불편한 상태였습니다.

### 보완

- 390px 모바일에서는 채널 주소 입력칸과 `YouTube API로 확인` 버튼을 위아래로 배치합니다.
- 입력칸과 버튼은 모바일에서 각각 한 줄 전체 폭을 사용하고, 640px 이상에서는 기존 한 줄 배치를 유지합니다.
- 단일 채널 확인·Azure DB 저장·영상 수집 함수는 변경하지 않았습니다.

### 검수 범위

- 로컬 390×844 화면에서 임시 문자열만 입력했으며 확인 버튼은 누르지 않았습니다.
- YouTube API·Azure DB 조회·저장·영상 수집은 실행하지 않았습니다.

## 256. 2026-08-12 수집 영상 적용 필터의 모바일 줄바꿈 보강

### 재점검

- 채널 태그별 보기의 대량 결과는 처음 60개만 표시하고 60개씩 더 보여주는 기존 화면 계산을 유지합니다.
- 더 보기는 Azure DB 재조회·저장이나 YouTube API 호출을 실행하지 않습니다.
- 조회 성공 후 실제 0개와 조회 전·조회 실패 상태도 서로 다른 안내와 다음 행동을 유지합니다.

### 보완

- 수집 영상 목록으로 복귀했을 때 검색어가 매우 길면 적용 필터 배지가 모바일 폭을 밀 수 있는 여지를 확인했습니다.
- 적용 필터 묶음과 배지에 최대 폭·최소 폭 기준을 주고 긴 검색어를 여러 줄로 표시하도록 했습니다.

### 로컬 검수

- 390×844에서 긴 영문 검색어를 입력했을 때 배지는 275px 안에서 66px 높이로 줄바꿈됐습니다.
- 문서 표시 폭과 스크롤 폭은 모두 375px로 같았고 콘솔 오류가 없었습니다.
- 로컬 임시 입력만 사용했으며 Azure DB·YouTube API·저장·영상 수집은 실행하지 않았습니다.

## 257. 2026-08-12 대량 목록의 필터 결과와 단계 표시 수 구분

### 운영 확인

- 정식 운영 주소 `creator.kaion.co.kr` 로그인과 `creator_owner` 권한 적용을 확인했습니다.
- 온라인 저장소(Azure DB)에서 채널 21개·보관 소재 1개·링크 후보 1개를 확인했습니다.
- David Fortin 한 채널을 선택해 Azure DB 읽기 1회로 수집 영상 정보 251개를 불러왔습니다.
- 화면에는 처음 60개와 `영상 60개 더 보기 (60/251)`가 정상 표시됐고 390px 가로 넘침과 콘솔 오류가 없었습니다.

### 확인한 혼동과 보완

- 같은 화면 상단의 `현재 표시 251개 / 전체 251개`는 실제 60개 단계 표시와 모순되어 보였습니다.
- 상단 숫자의 실제 의미에 맞게 `필터 결과 251개 / 불러온 전체 251개`로 바꿨습니다.
- URL 목록 복사는 화면에 만든 60개 카드만이 아니라 현재 필터 조건에 맞는 전체 결과를 복사한다는 설명으로 바로잡았습니다.

### 데이터 경계

- 채널 선택은 현재 브라우저의 조회 범위만 변경했습니다.
- Azure DB 조회 1회 외에 저장·수정·삭제는 없었고 YouTube API·댓글 API·새 영상 수집은 실행하지 않았습니다.

## 258. 2026-08-12 향후 기능 메뉴 통합

### 확인한 중복

- 후킹·기승전결·와우포인트·대본 생성·제목/썸네일·재구성·복사 위험은 별도 메뉴보다 대본 작업실의 단계별 도구에 가깝습니다.
- 제작 상태판은 제작 후보함·대본 작업실·업로드 캘린더의 현재 상태와 역할이 겹칩니다.
- 트렌드, API 사용량, 성과·패턴 리포트도 각각 기존 키워드 탐색, 최근 수집 상태·설정, 인사이트 영역 안에서 제공하는 편이 흐름이 단순합니다.

### 변경

- 사이드바의 향후 기능을 13개 개별 항목에서 `키워드 트렌드 신호`, `대본 작업실 확장 도구`, `수집·API 사용 현황`, `성과·패턴 인사이트` 4개 통합 목적지로 정리했습니다.
- 과거 11개 준비중 ID는 호환 목록에 보존해 오래된 링크가 홈으로 잘못 돌아가지 않고 기존 준비중 안내를 계속 표시합니다.
- 실제 기능, API endpoint, Azure DB, localStorage, YouTube API 호출 조건은 변경하지 않았습니다.

## 259. 2026-08-12 대본 작업실 수동 작성 기준

### 목적

- AI 기능을 연결하기 전에도 영상 원본을 분석하고 실제 대본으로 발전시킬 수 있는 최소 작업 기준을 제공합니다.
- 새 입력 필드나 저장 구조를 늘리지 않고 현재 영상 분석·구성안·대본 본문 입력칸을 더 잘 쓰도록 돕습니다.

### 변경

- 대본 편집기 상단에 기본적으로 접힌 `AI 없이 작성하는 대본 작업 기준`을 추가했습니다.
- 원본 분석, 구성안, 대본 본문 세 단계마다 실무 점검 항목 5개를 제공합니다.
- 오래된 정보·정책·가격 확인, 원본과 지나치게 비슷한 표현 제거, 제목의 약속과 결론 일치도 포함했습니다.

### 데이터 경계

- 안내를 펼치고 접는 것은 화면 표시만 바꿉니다.
- 기존 `videoUserRecords` 필드와 저장 버튼을 그대로 사용하며 Azure DB·localStorage·YouTube API·AI endpoint를 변경하거나 호출하지 않습니다.

## 260. 2026-08-12 기존 작업 묶음의 수동 AI 전달 명확화

- 카이온 자원 원장을 확인한 결과 Creator OS는 ChatGPT 구독을 수동 기획 확장에 우선 활용하고, OpenAI API는 자동·반복 처리가 필요할 때만 검토하는 정책입니다.
- 기존 `작업 묶음 복사`가 필요한 제작 정보를 이미 모두 포함하므로 별도 AI 버튼이나 endpoint를 중복 추가하지 않았습니다.
- 버튼을 `AI 작업 자료 복사`로 명확히 하고, 복사 내용에 복제 금지·확인 필요 사실 표시·근거 없는 정보 생성 금지·미작성 부분 우선 제안 요청을 포함했습니다.
- 사용자가 복사한 뒤 ChatGPT 등 선택한 작업 공간에 직접 붙여넣는 수동 흐름이며 앱은 AI API를 호출하거나 Azure DB에 새로 저장하지 않습니다.

## 261. 2026-08-12 운영 AI 복사·수집/API 경계 후속 검수

- 정식 운영 제작 후보함에서 `AI 작업 자료 복사` 버튼, 설명, 복사 완료 상태를 확인했습니다.
- 실제 클립보드에는 AI 작업 요청, 원본 제목·채널·URL, 제작 단계, 대본 단계, 분석·구성안·본문·메모가 포함됐습니다.
- 운영 최근 수집 상태는 Azure DB의 21개 채널과 최근 수집 이력을 정상 조회했고, 화면 자체는 YouTube API나 DB 저장을 실행하지 않았습니다.
- 별도 사용량 메뉴를 만들지 않고 현재 화면에 `현재 화면 조회`, `새 영상 수집`, `정확한 API 사용량`의 세 가지 경계를 표시했습니다.
- 과거 영상 수동 수집의 자동 반복·예약 없음 안내가 최신 영상 `예약 수집` 이력과 충돌해 보이지 않도록 서로 다른 실행임을 명시했습니다.
- 정확한 YouTube API 쿼터 장부는 추가하지 않았으며 실제 사용량은 Google Cloud Console이 최종 기준입니다.

## 262. 2026-08-12 키워드 트렌드 신호의 기존 도구 통합

- 운영 키워드 탐색에서 외부 관심도 도구가 `수집 영상에서 찾기`에만 있고 실제 새 키워드를 입력하는 `YouTube에서 새로 찾기`에는 없는 비대칭을 확인했습니다.
- 새 트렌드 API나 별도 메뉴를 만들지 않고 기존 Google Trends·네이버 DataLab·네이버 검색광고 바로가기를 영상 검색과 채널 검색에서 함께 사용합니다.
- Google Trends에는 현재 YouTube 검색 지역을 전달하고, 검색 지역이 전체이면 대한민국을 기본 지역으로 명시합니다.
- 네이버 도구는 국내 관심도 확인용이며 키워드 복사 후 직접 확인하는 기존 흐름을 유지합니다.
- 현재 조회수·구독자·게시 후 하루 평균은 실시간 추세가 아니므로 앱 내부 급상승 자동 판정으로 확대하지 않습니다.
- 외부 공식 사이트를 여는 링크만 추가하며 YouTube API·Azure DB·localStorage·새 유료 서비스는 변경하지 않습니다.
## 263. 2026-08-12 YouTube 임시 검색 결과 요약과 중요 채널 연결 보강

- 영상 검색 결과에 최근 7일·30일·60일 결과 수, 출처 채널 수, 미등록 채널 수, 평균 조회수를 한 번에 보는 요약을 추가했습니다.
- 이 요약은 현재 받아온 최대 25개씩의 임시 결과와 화면 조회수 필터만 계산합니다. YouTube 전체 검색량, 실제 급상승 또는 시장 추세로 판정하지 않습니다.
- `미등록 채널 영상만 보기`는 이미 받은 결과의 출처 채널 등록 상태만 화면에서 좁히며 YouTube API나 Azure DB를 호출하지 않습니다.
- 영상 카드의 `이 채널 등록 검토`와 `YouTube 원본 보기`를 모바일에서 한 줄씩 안정적으로 누를 수 있게 배치했습니다. 등록 검토는 채널 운영실 입력 준비만 하며 자동 등록·새 영상 수집·DB 저장을 실행하지 않습니다.
- 전용 계산 테스트와 화면 렌더링 테스트를 추가했습니다.

## 264. 2026-08-12 반복 등장 미등록 채널 등록 검토 연결

- 현재 받아온 영상 검색 결과에 같은 미등록 출처 채널의 영상이 2개 이상 있으면 최대 3개까지 별도 묶음으로 표시합니다.
- 영상 수가 많은 채널을 먼저 보여주고 같으면 현재 결과의 조회수 합계를 참고해 순서를 정합니다. 이 순서는 현재 결과 안의 검토 편의 기준이며 성장률·채널 품질 점수가 아닙니다.
- 채널 버튼은 기존 채널 운영실 등록 입력을 준비하며 YouTube API 확인·Azure DB 저장·새 영상 수집을 자동 실행하지 않습니다.
- 자동 테스트에서 동일 채널 영상 2개가 `영상 2개 · 등록 검토`로 표시되고 이미 등록된 채널은 반복 후보에서 제외되는 계산을 확인했습니다.

## 265. 2026-08-13 한글 포함 제목 화면 필터

### 운영 근거

- `MS Copilot`, 대한민국, 한국어 우선, 최근 30일 검색 1회에서 영상 25개와 출처 채널 19개를 확인했습니다.
- 한국어 우선은 한국어만 제한하는 조건이 아니어서 영어 제목도 정상적으로 포함됐습니다.

### 변경

- 현재 받은 영상 제목을 `전체 제목`, `한글 포함 제목`, `한글 없는 제목`으로 좁히는 화면 필터를 추가했습니다.
- 한글 완성형과 자모를 문자 범위로 검사하며 실제 영상의 음성·자막 언어는 판정하지 않습니다.
- 채널 등록 상태 필터와 조합되고 임시 검색 세션 이동·복귀 시 유지되며, 임시 결과 지우기에서 함께 초기화됩니다.

### 데이터 경계

- 화면 배열만 필터링합니다.
- 필터 변경 자체는 YouTube API·Azure DB 조회·저장·새 영상 수집을 실행하지 않습니다.

## 266. 2026-08-13 검색 결과 필터·정렬·저장 역할 분리

### 확인한 문제

- 등록 상태와 제목 필터가 저장 분류·발견 링크 저장 버튼과 한 영역에 있어 화면 조회와 DB 저장 동작이 섞여 보였습니다.
- YouTube API 검색 정렬 외에 이미 받은 결과를 비교하기 위한 화면 정렬이 없었습니다.
- YouTube 검색 통신 실패는 공통 로그인 복구 UI가 있음에도 검색 화면에서 원문 오류만 표시했습니다.

### 변경

- `결과 좁히기`에 등록 상태, 제목 문자, 현재 결과 정렬, 적용 배지, 화면 필터 초기화를 모았습니다.
- `선택 영상 저장 설정`에 저장 분류, 선택 개수, 발견 링크함 저장·이동만 배치했습니다.
- 현재 결과는 받은 순서, 조회수, 대박 비율, 하루 평균, 최신순으로 화면에서 재정렬합니다.
- 한글 포함 결과가 0개이면 한국어 우선 검색의 한계를 설명하고 전체 결과로 돌아가는 동작을 제공합니다.
- 검색 오류는 기존 `CreatorActionFeedback`을 사용해 로그인 만료 가능성과 Microsoft 로그인 다시 열기를 안내합니다.

### 데이터 경계

- 필터·정렬·배지·초기화는 현재 브라우저의 임시 결과만 변경합니다.
- 발견 링크함 저장 버튼을 실제로 누르기 전까지 Azure DB 쓰기는 없으며, 결과 정렬로 YouTube API를 다시 호출하지 않습니다.

## 267. 2026-08-13 채널 검색 로그인 복구 안내 통일

- 같은 키워드 탐색 안에서 영상 검색만 공통 로그인 복구 안내를 사용하고 채널 검색은 원문 오류만 표시하던 불일치를 보완했습니다.
- 채널 검색도 기존 `CreatorActionFeedback`을 사용해 로그인 만료 가능성과 Microsoft 로그인 다시 열기를 안내합니다.
- 로그인 다시 열기는 인증 화면 이동만 수행하며 YouTube API 호출이나 Azure DB 조회·저장을 실행하지 않습니다.

## 268. 2026-08-13 영상 검색 중요 채널 모음과 임시 세션 복구

### 중요 채널 모음

- 영상 카드에서 영상 후보 선택과 별도로 출처 채널을 `중요 채널` 후보로 선택합니다.
- 같은 채널의 영상이 여러 개여도 채널은 한 번만 계산하고, 표시된 미등록 출처 채널을 한 번에 선택할 수 있습니다.
- 최대 50개를 기존 채널 운영실 일괄 등록 검토로 전달하며, 전달 단계는 주소 입력만 준비합니다.
- 최종 `YouTube API 확인 후 일괄 저장`을 사용자가 직접 누르기 전에는 API 확인·Azure DB 쓰기·새 영상 수집이 없습니다.

### 새로고침 복구

- 키워드 탐색의 영상·채널 검색 조건, 임시 결과, 화면 필터와 영상/채널 선택을 `sessionStorage`에 최대 6시간 보관합니다.
- 같은 탭에서 새로고침하면 자동 검색 없이 복구하고, 탭 종료 또는 6시간 경과 시 복구하지 않습니다.
- 저장소를 사용할 수 없거나 용량 제한이 발생해도 검색 기능은 그대로 동작하고 복구만 생략합니다.
- Azure DB·localStorage·YouTube API 계약과 프로젝트 자원 선언은 변경하지 않습니다.

## 269. 2026-08-13 중요 채널 모음 운영 왕복 검수

- PR #1125와 main `0ce4f08` 배포 성공 후 정식 운영 화면을 점검했습니다.
- `MS Copilot`·대한민국·한국어 우선·최근 30일 검색을 정확히 1회 실행해 영상 25개를 받았습니다.
- 영상 후보와 별도로 서로 다른 중요 출처 채널 2개를 선택했고, 채널 운영실 일괄 등록 검토에서 `2/50개 인식됨`을 확인했습니다.
- 검색 결과로 돌아온 뒤 결과 25개와 선택 2개가 유지됐고, 새로고침 뒤 복원 안내·조건·결과·선택이 다시 표시됐습니다. 복원 중 추가 검색 버튼은 누르지 않았습니다.
...
## 271. 2026-08-13 검색 영상 중복 안내 운영 검수

- PR #1127과 main `d18275f` 배포 성공 후 기존 `MS Copilot` 임시 결과 25개를 재사용했습니다.
- 새 YouTube 검색 없이 발견 링크함 기존 저장 3개와 새로 저장할 선택 0개가 화면에 표시됐습니다.
- 390×844에서 viewport 390px·문서 너비 375px로 가로 넘침이 없었고 브라우저 error 로그는 0건이었습니다.
- 발견 링크 저장 버튼을 누르지 않아 Azure DB 쓰기는 없었고, 기존 목록 조회 외에 새 데이터 작업은 실행하지 않았습니다.

## 272. 2026-08-13 발견 링크 제작 흐름 연속성 보강

- 검색 영상 저장 payload의 `linkedVideoId`·URL·제목·태그가 제작 후보 발견 링크와 대본 작업 원본까지 이어지는 코드를 대조했습니다.
- 제작 후보함과 대본 작업실은 발견 링크 ID를 기준으로 같은 원본을 찾으며, 대본 기록에는 `discovery-link:{ID}`를 사용해 수집 영상 ID와 충돌하지 않습니다.
- 기존 제작 후보 링크가 발견 링크함에서 후보함으로 다시 가는 버튼이 없어 `후보함에서 보기`를 추가했습니다.
- 후보함의 특정 링크→대본 작업실 특정 원본 전달을 포함한 통합 테스트를 추가했습니다.
- 화면 이동과 자동 테스트만 수행하며 YouTube API·Azure DB 읽기/쓰기·localStorage 변경은 추가하지 않았습니다.

## 273. 2026-08-13 발견 링크 후보→대본 운영 이동 검수

- PR #1129 배포 후 기존 운영 표본 `Codex 임시 흐름 검증 2026-07-20`을 사용했습니다.
- 발견 링크함의 `후보함에서 보기`가 제작 후보함의 해당 링크 한 건만 표시하고, `대본 작업 시작`이 같은 원본의 대본 작업실을 여는 것을 확인했습니다.
- 390×844에서 viewport 390px·문서 너비 375px로 가로 넘침이 없고 브라우저 error 로그는 0건이었습니다.
- 상태 변경, 후보 저장, 대본 저장, YouTube API 호출은 실행하지 않았습니다. 발견 링크 목록 읽기와 화면 이동만 수행했습니다.
- 계속 진행목록은 10개 단위로 관리하며 1~4번 완료, 5~9번 자동 진행, 10번 실제 저장 왕복은 사용자 대상 결정으로 분리했습니다.

## 274. 2026-08-13 계속 진행목록 5~9번 검색 업무 화면 정리

- 모바일 검색 카드의 제목·지표·결정 버튼 간격을 줄이고 계산식은 접이식 상세로 이동했습니다.
- 영상 후보와 중요 채널은 별도 결정임을 카드에 먼저 안내하고, 모바일에서 반복 설명은 숨기되 버튼의 데이터 경계 설명은 유지합니다.
- `카이온학습 (영상 링크 분류)`와 채널 분야 태그의 차이를 저장 영역에 표시했습니다.
- 같은 채널의 여러 검색 영상은 채널 ID 기준 한 등록 후보로 합치며 이미 등록된 채널은 제외한다는 안내와 중복 입력 회귀 테스트를 추가했습니다.
- YouTube 새 검색 작업을 Azure DB 중복 조회, YouTube API 검색, Azure DB 선택 저장, 채널 운영실 화면 이동으로 나눠 한 안내에 표시했습니다.
- 수집 영상에서 찾기와 YouTube에서 새로 찾기는 각각 기존 수집 정보 재탐색과 새 후보 발굴이라는 순서를 소스 선택부에 설명했습니다.
- 새 자원·새 Cosmos DB container·localStorage 기준 데이터·자동 YouTube API 요청은 추가하지 않았습니다.
- 다음 작업은 배포 후 모바일 화면 검수와 테스트 링크 1건의 Azure DB 저장·재조회·삭제 원상복구입니다.

## 275. 2026-08-13 계속 진행목록 10번 Azure DB 저장·원상복구 검수

- 검수 전 발견 링크 수는 6개였습니다.
- `Codex Azure DB 왕복 검수 2026-08-13` 링크를 기존 발견 링크함에 저장해 7개가 되고, 재조회 뒤 같은 제목이 정확히 1건 유지되는 것을 확인했습니다.
- 사용자 삭제 승인 후 제목과 URL이 모두 일치하는 검수 문서 1건만 삭제했습니다.
- Azure DB 재확인에서 검수 문서는 0건, 같은 사용자의 발견 링크는 원래 6개로 복원됐습니다.
- 새 YouTube 검색, 채널 등록, 새 영상 수집, 다른 발견 링크 수정, 새 Azure 자원·Cosmos DB container 생성은 실행하지 않았습니다.

## 276. 2026-08-13 쇼츠 후보 전용 탐색

- `YouTube에서 새로 찾기`에 `쇼츠 후보 (3분 이하)` 길이 조건과 `쇼츠 후보 전용` 빠른 설정을 추가했습니다.
- 서버 요청은 기존 `videoDuration=short` 계약을 재사용하고, 프론트가 `durationSeconds <= 180`인 결과만 남깁니다.
- API만으로 영상 화면비를 확정할 수 없어 확정 쇼츠가 아닌 후보로 안내하고 카드에 `쇼츠 후보` 배지를 표시합니다.
- 후보가 없는 API 페이지에서도 다음 페이지 토큰을 유지해 사용자가 다음 25개를 명시적으로 계속 조회할 수 있습니다.
- 기존 백엔드와 수집 영상의 쇼츠 분류는 변경하지 않았습니다. 전체 프론트 테스트 1,267개와 production 빌드가 통과했습니다.
- 로컬 390×844에서 쇼츠 후보 조건·빠른 설정·한계 안내가 보이고 문서 너비와 viewport가 375px로 일치하며 브라우저 오류가 없음을 확인했습니다. 실제 YouTube 검색과 Azure DB 작업은 실행하지 않았습니다.

## 277. 2026-08-13 쇼츠 후보 가능성 구분

- 3분 이하 후보를 제목·설명의 명시적 Shorts 표기에 따라 `쇼츠 가능성 높음`과 `쇼츠 확인 필요`로 구분했습니다.
- 결과 요약 개수, `가능성 높음만`·`확인 필요만` 화면 필터, 카드별 근거와 YouTube 원본 확인 동선을 추가했습니다.
- 제목·설명 표기는 보조 신호일 뿐 확정 판정이 아니며, 화면비와 YouTube의 실제 Shorts 분류는 원본 확인 대상으로 유지합니다.
- 현재 API 응답만 화면에서 계산하므로 추가 YouTube API 호출·Azure DB 읽기/쓰기·새 자원·container 변경은 없습니다.
- 전체 프론트 테스트 1,269개와 production 빌드가 통과했습니다. 로컬 390×844에서 문서 너비와 viewport가 375px로 일치하고 브라우저 오류가 없었습니다. 실제 YouTube 검색은 실행하지 않았습니다.

## 278. 2026-08-14 YouTube 검색 영상 게시 시각 표시

- 검색 응답의 기존 `publishedAt`을 한국 시간(`Asia/Seoul`) 기준 날짜·오전/오후·시·분으로 표시합니다.
- 같은 줄에 `몇 분 전`, `몇 시간 전`, `몇 일 전` 경과 시간을 함께 보여 게시 직후 반응 속도를 판단하기 쉽게 했습니다.
- 날짜·시간 값이 없거나 잘못된 경우 `게시 시각 미상`으로 안전하게 표시합니다.
- 화면 표시만 바꾸며 추가 YouTube API 호출·Azure DB 읽기/쓰기·새 저장 필드·새 자원은 없습니다.
- 전체 테스트 2,463개(검수용 워크트리 중복 포함)와 production 빌드가 통과했습니다.
- 로컬 390×844 YouTube 탐색 화면에서 viewport와 문서 너비가 375px로 일치하고 브라우저 오류가 없었습니다. 실제 YouTube 검색은 실행하지 않았습니다.
- 첫 GitHub Linux CI에서 `ko-KR` 날짜 안의 오전·오후가 `AM/PM`으로 반환되는 환경 차이를 확인했습니다. 사용자 승인 후 해당 표기만 `오전/오후`로 정규화해 Windows와 Linux의 표시 계약을 같게 맞췄습니다.
