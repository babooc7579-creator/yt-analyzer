...
- 2026-07-13 전체 검증 기준은 테스트 파일 181개, 테스트 792개 통과와 프로덕션 빌드 성공입니다.
...
- 2026-07-13 백엔드 PR #12와 프론트엔드 PR #885 병합 후 Function App과 Azure Static Web Apps 배포가 성공했습니다. 보호된 운영 API의 자동 왕복 smoke는 인증 경계 때문에 완료하지 못했으며 임시 테스트 기록은 만들지 않았습니다. owner 계정의 실제 사용 확인을 최종 운영 점검으로 남깁니다.
- 2026-07-13 홈에 `오늘 집중` 수와 제작 후보함 바로가기 흐름을 연결했습니다. 숫자는 Cloud `videoUserRecords.focusPinnedAt`을 읽어 계산하며, 홈 표시와 화면 이동만으로 저장이나 YouTube API 호출은 실행하지 않습니다. 제작 후보 저장 중에는 같은 영상의 집중/상태 요청을 중복 전송하지 않고, 예외가 나면 실패 상태를 표시합니다.
- 2026-07-13 `터또터 탐색`과 `오늘 볼 채널`을 준비중 메뉴에서 정식 실사용 화면으로 전환했습니다. 터또터는 저장 영상 중 6개월 이상·채널 평균 대비 1.5배 이상 후보만 다루며, 오늘 볼 채널은 운영중 채널을 미수집·등급·마지막 수집일 기준으로 정리합니다. 두 화면의 검색·필터·선택만으로 YouTube API를 호출하지 않습니다.
- 2026-07-13 제작 후보함에 저장된 작업 검색과 진행 단계 필터를 추가했습니다. 영상 제목·채널·제목 초안·메모 및 발견 링크를 찾을 수 있고, 필터 초기화는 화면 상태만 바꾸며 Cloud 데이터를 수정하지 않습니다.
- 2026-07-13 제작 후보 영상 카드에 `작업 묶음 복사`를 추가했습니다. 현재 카드의 진행 단계, 원본/초안 제목, 채널, 원본 URL, 대박 지수, 예정일, 준비 상태, 제작 메모를 로컬 클립보드로 복사하며 Cloud 저장이나 YouTube API 호출은 실행하지 않습니다. 전체 검증은 테스트 파일 181개, 테스트 792개와 production build 통과입니다.
...
- 2026-07-14 업로드 캘린더에서 제작 후보함으로 넘어갈 때 제목만 검색하던 흐름을 영상 ID 기준 한 건 조회로 보강했습니다. 후보함은 캘린더에서 가져온 검색임을 표시하고 전체 작업 보기와 캘린더 복귀를 제공하며, 이 이동/필터 상태는 Cloud에 저장되지 않습니다. 홈 레이더가 비었을 때는 채널 미선택과 저장 영상 미조회 상태를 구분해 다음 행동을 안내합니다. 영상 카드는 게시일과 경과일을 함께 표시하고, 후보 이유와 제작 후보 행동을 먼저 판단하기 쉬운 순서로 정리했습니다. 영상 썸네일은 최대 해상도를 우선 사용하고 없는 경우 자동으로 다음 화질로 대체합니다.
- 2026-07-15 설정 화면에서 기존 `GET /channels`를 이용한 수동 Cloud 채널 재조회, 오류별 다음 행동 안내, 숨긴 분야 복원을 추가했습니다. 오늘의 레이더는 다른 화면에서 돌아와도 현재 선택 채널과 일치하는 저장 영상만 이어서 사용합니다. 전체 검증은 테스트 파일 208개, 테스트 884개와 production build를 통과했고 PR #906 배포가 완료됐습니다.
- 2026-07-15 설정 화면에서 기존 Cloud 채널 조회를 수동 재실행하고, 로그인/연결/CORS 가능성/서버 오류에 맞는 다음 행동을 확인할 수 있도록 보강했습니다. 숨긴 기본 분야와 Cloud 태그는 기존 카테고리 localStorage key를 그대로 사용해 화면 목록에 복원할 수 있습니다. 오늘의 레이더는 다른 화면을 보고 돌아와도 현재 선택 채널과 일치하는 불러온 영상만 계산해 후보 판정 흐름을 이어갑니다. endpoint, DB schema, localStorage key, YouTube API 호출 조건은 변경하지 않았습니다.
...
- 현재 상태: 2026-07-26 승인된 B안 구현 완료. 기존 전체 조회는 호환 유지하고 선택형 페이지 조회를 추가했습니다.
- 안정화 상태: 페이지 수·누적 영상 수·경과 시간 표시, 실패 재시도, 성공한 빈 결과 안내를 보강했습니다. 키워드 탐색·태그별 금고·터또터 탐색의 실패/빈 결과도 같은 Cloud DB 원칙으로 테스트했고, 1,000개 영상 기준 검색·필터·정렬 검증을 추가했습니다.
- 왜 필요한가: 채널과 영상이 늘면 앱 첫 사용감이 무거워질 수 있습니다.
- 작업 범위: 백엔드는 `pageSize`/`continuationToken`을 지원하고, 프론트는 모든 페이지를 순차 조회해 기존 전체 목록 의미를 유지합니다.
- 건드릴 파일 예상: 백엔드 videos endpoint, 프론트 영상 로딩 로직.
- 건드리면 안 되는 것: 기존 무페이지 endpoint 계약, 정렬/필터의 전체 데이터 기준.
- 위험도: 중간.
- 확인 결과: 운영 데이터 기준 저장 영상 1,821개, `/videos` 응답 약 1.16MB입니다.
- 완료 기준: 기존 호출 호환, 페이지 연속 조회, 중간 실패 시 부분 데이터 미노출, 전체 테스트와 배포 확인.
- 다시 검토할 조건: 전체 페이지 조회 자체가 느려지거나 응답량이 커질 때 서버 검색/정렬 또는 화면 가상화를 별도 판단합니다.
- 사용자 판단 필요 여부: 완료됨. 서버 검색/정렬과 가상화는 별도 제품 판단 필요.
...
- 운영 동선: 최근 상태의 채널별 `이 채널 관리`와 실패·부분 성공·미수집 채널의 `수집 단계 열기`는 해당 채널 하나를 현재 선택한 뒤 채널 운영실의 해당 단계로 이동합니다. 이동만으로 Cloud 저장이나 YouTube API 수집은 실행되지 않습니다.
- 이력 후속 동선: 과거 수집 이력의 채널별 결과에서도 `채널 관리`로 해당 채널을 선택해 운영실로 이동합니다. 이력이 비어 있으면 `수집할 채널 고르기`로 채널 관리 단계에 먼저 이동해 범위를 정하게 하며 실제 수집은 자동 실행하지 않습니다.

### Issue 9. discovery links/local assets MVP 범위 결정

- 목적: 인스타/외부 링크와 다운로드한 로컬 파일을 안전하게 연결하는 최소 범위를 정합니다.
- 현재 상태: 2026-07-22 기준 discovery links 수동 저장 1차 MVP와 제작 후보함 왕복 연결이 구현되었고, local assets는 아직 목표 모델 단계입니다.
...
- 추가 상태 3: 제작 후보함의 `발견함에서 수정`은 링크 ID를 일회성 화면 의도로 전달해 발견함에서 정확한 한 건을 보여줍니다. 발견함 상단에서 같은 후보로 돌아가거나 전체 링크 보기로 전환할 수 있으며, 이 이동은 Cloud 저장, 외부 수집, 자동 병합을 실행하지 않습니다.
...
- 현재 상태: 2026-07-06 사용자 승인 후 `vitest`와 `test: vitest run` 스크립트를 추가했습니다. 2026-07-22 기준 `npm test -- --run`에서 테스트 파일 220개, 테스트 961개가 통과합니다. GitHub Actions `Build` workflow는 `npm test` 후 `npm run build`를 실행합니다. 영상 보관에서 제작 후보, 오늘 집중, 업로드 일정으로 이어지는 흐름과 발견 링크 후보의 분리 저장 계약, 레이더·스크랩북·발견 링크 판단 후 제작 후보함 인계와 원래 화면 복귀 흐름도 회귀 테스트로 고정했습니다.
...
- 2026-07-26 오늘의 레이더 4단계 진행률, 현재 할 일, 제작 후보·오늘 집중 인계 표시를 보강했습니다. 오늘 채널 선택은 화면 이동 중 유지되는 세션 상태이며 브라우저 새로고침 시 초기화됩니다. 영상 판단과 제작 후보 기록은 기존 Cloud 기준을 유지합니다. 레이더·홈 지표·제작 칸반의 제작 후보 수 연결을 계약 테스트로 고정했고, 전체 테스트 223개 파일, 1,014개 테스트와 production build를 통과했습니다.

다음 기능 개선 후보:

1. 최근 수집 상태는 부분 성공의 저장 범위와 실패 원인별 다음 행동까지 표시합니다. 과거 누락 영상 전체를 채우는 보강 수집은 현재 기능이 아니므로 자동 재시도나 반복 수집으로 대신하지 않습니다.
2. owner 계정으로 제작 후보 1개를 `오늘 집중`으로 고정한 뒤 새로고침 후 유지되는지 실제 사용 확인합니다. 자동 smoke에서는 인증 경계 때문에 Cloud 왕복을 만들지 않았습니다.
3. `오늘 집중`은 수동 고정 순서와 홈 바로가기까지 구현됐습니다. 1차 실사용 후 집중 후보 수 제한, 끌어놓기 재정렬, 링크 후보 포함이 실제로 필요한지 관찰하며, 저장 의미가 바뀌는 확장은 별도 판단으로 진행합니다.
4. `/videos` 선택형 페이지네이션은 구현 완료됐습니다. 다음 성능 확장은 실사용 측정 후 서버 검색/정렬 또는 화면 가상화를 별도 판단합니다.
5. local assets와 별도 제작 프로젝트 모델은 1차 실사용 후 필요성이 확인될 때 다시 검토합니다.
6. 위 항목 중 DB schema, endpoint, localStorage key, YouTube API 호출량이 바뀌는 순간에는 별도 선택지 보고 후 멈춥니다.
