...
- 2026-07-14 업로드 캘린더에서 제작 후보함으로 넘어갈 때 제목만 검색하던 흐름을 영상 ID 기준 한 건 조회로 보강했습니다. 후보함은 캘린더에서 가져온 검색임을 표시하고 전체 작업 보기와 캘린더 복귀를 제공하며, 이 이동/필터 상태는 Cloud에 저장되지 않습니다. 홈 레이더가 비었을 때는 채널 미선택과 저장 영상 미조회 상태를 구분해 다음 행동을 안내합니다. 영상 카드는 게시일과 경과일을 함께 표시하고, 후보 이유와 제작 후보 행동을 먼저 판단하기 쉬운 순서로 정리했습니다. 전체 검증은 테스트 파일 196개, 테스트 841개와 production build를 통과했습니다.
...
- 현재 상태: 2026-07-06 사용자 승인 후 `vitest`와 `test: vitest run` 스크립트를 추가했습니다. 2026-07-14 기준 `npm.cmd test`에서 테스트 파일 196개, 테스트 841개가 통과합니다. GitHub Actions `Build` workflow는 `npm test` 후 `npm run build`를 실행합니다.
