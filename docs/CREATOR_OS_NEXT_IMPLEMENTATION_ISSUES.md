...
- 위험도: 낮음.
- 완료 기준: 1차 완료. DB 조회 버튼과 YouTube API 호출 버튼이 주요 흐름에서 구분됩니다.
...
1. Issue 10: 제작 후보와 제작 칸반의 데이터 기준 결정
2. discovery links 구현 전 저장 위치와 상태값 선택지 보고
3. `scan_logs`와 `api_quota_logs` 실제 구현 여부 선택지 보고

이 순서가 안전한 이유:

- 문서 기준, 저장 영상 페이지네이션 감사, scan/API 사용 기록 모델, discovery MVP 범위 검토는 완료됐습니다.
- 다음은 제작 후보 데이터 기준을 결정합니다.
