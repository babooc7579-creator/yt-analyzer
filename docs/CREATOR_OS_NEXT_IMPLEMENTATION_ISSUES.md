...
- 현재 상태: 2026-07-06 사용자 승인 후 `vitest`와 `test: vitest run` 스크립트를 추가했습니다. 2026-07-08 기준 `npm.cmd test`에서 테스트 파일 105개, 테스트 468개가 통과합니다. `src/utils` 순수 유틸 93개와 테스트 파일 93개가 1:1로 확인됐고, GitHub Actions `Build` workflow는 `npm test` 후 `npm run build`를 실행합니다.
