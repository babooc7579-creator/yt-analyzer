...
1. owner 계정으로 제작 후보 1개를 `오늘 집중`으로 고정한 뒤 새로고침 후 유지되는지 실제 사용 확인합니다. 자동 smoke에서는 인증 경계 때문에 Cloud 왕복을 만들지 않았습니다.
2. `오늘 집중`은 수동 고정 순서와 홈 바로가기까지 구현됐습니다. 1차 실사용 후 집중 후보 수 제한, 끌어놓기 재정렬, 링크 후보 포함이 실제로 필요한지 관찰하며, 저장 의미가 바뀌는 확장은 별도 판단으로 진행합니다.
3. `/videos` 선택형 페이지네이션은 구현 완료됐습니다. 다음 성능 확장은 실사용 측정 후 서버 검색/정렬 또는 화면 가상화를 별도 판단합니다.
