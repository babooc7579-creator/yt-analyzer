...
- 현재 상태: 2026-07-06 사용자 승인 후 `vitest`와 `test: vitest run` 스크립트를 추가했습니다. 2026-07-21 기준 `npm test -- --reporter=dot`에서 테스트 파일 220개, 테스트 948개가 통과합니다. GitHub Actions `Build` workflow는 `npm test` 후 `npm run build`를 실행합니다. 영상 보관에서 제작 후보, 오늘 집중, 업로드 일정으로 이어지는 흐름과 발견 링크 후보의 분리 저장 계약, 레이더 판단 후 처리 기록 복구와 제작 후보함 인계 흐름도 회귀 테스트로 고정했습니다.
