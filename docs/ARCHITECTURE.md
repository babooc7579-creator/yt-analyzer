...
3분 이하 후보는 현재 응답에 이미 포함된 제목·설명만으로 보조 분류합니다. `#shorts`, `shorts`, `쇼츠`, `ショート` 표기가 있으면 `쇼츠 가능성 높음`, 없으면 `쇼츠 확인 필요`로 표시합니다. 이 값은 YouTube의 공식 Shorts 분류나 화면비 판정이 아니며, 사용자는 화면 필터로 우선순위를 좁힌 뒤 기존 YouTube 원본 링크에서 최종 확인합니다. 보조 분류와 필터는 추가 API 호출·Azure DB 저장·새 세션 저장소를 만들지 않습니다.
